...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "companyproj-ear.ear");

        // Current directory is the root of the "companyproj-web" project. Go up one level
        // and enter the "companyproj-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../companyproj-ejb/target/companyproj-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "companyproj-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/companyproj-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "companyproj-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
